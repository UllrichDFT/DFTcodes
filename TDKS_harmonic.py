...
# 
# The default output is the time-dependent dipole moment d(t).
# Optional additional output is an animated plot of the time-dependent 
# density n(x,t).
...
TSTEPS = 400   # number of time steps
DT = 0.1       # time step
...
Animation = 0  # if set to 1, then the code shows an animation of the density
               # oscillations. To see anything interesting, make the kick
               # strong enough, e.g., EFIELD = 0.5
...
while counter < TSTEPS-1:  
    counter += 1
    T = T + DT
    if (counter+1)%10==0: print('time =',round(T,5))
    Time[counter] = T
#    
...
#        
    ...
    if Animation==1: 
        ...
        # Real-time plotting to produce animated plot.
        # https://stackoverflow.com/a/63702988
        plt.figure(1)
        plt.cla() # Clear previous plots
        plt.axis([x[0],x[-1],0,1.25]) # Set the axis limits to always be the same
